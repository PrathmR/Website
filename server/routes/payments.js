...
router.post('/offline', protect, uploadPaymentScreenshot.single('screenshot'), async (req, res) => {
  try {
    ...
    // Store screenshot URL (Cloudinary URL if using cloud storage, otherwise local path)
    const screenshotUrl = screenshot.path || `/uploads/payments/${screenshot.filename}`;
